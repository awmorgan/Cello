...
static int Sudoku_Update(struct Sudoku *aux, int8_t sr[729], uint8_t sc[324],
                         int r, int v) {
...
static int Sudoku_Solve(struct Sudoku *aux, const char *_s) {
  ...
  for (i = 0; i < 81; ++i) {
    ...
    if (a >= 0)
      Sudoku_Update(aux, sr, sc, i * 9 + a, 1); // set the choice
    if (a >= 0)
  ...
  for (i = 0, dir = 1, cand = 10 << 16 | 0, out[81] = 0;;) {
    while (i >= 0 && i < 81 - hints) { // maximum 81-hints steps
      ...
      c = cc[i];
      if (dir == -1 && cr[i] >= 0)
        Sudoku_Update(aux, sr, sc, aux->r[c][cr[i]], -1); // revert the choice
