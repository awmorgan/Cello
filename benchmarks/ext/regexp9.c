 ...
 */
static Relist *_renewthread(Relist *lp, /* _relist to add to */
 ...
 */
static Relist *_renewemptythread(Relist *lp, /* _relist to add to */
                                 Reinst *ip, /* instruction to add */
                                 int ms,
...
static Relist *_rrenewemptythread(Relist *lp, /* _relist to add to */
                                  Reinst *ip, /* instruction to add */
                                  int ms,
...
extern Reprog *regcomp9(char *s) { return regcomp1(s, 0, ANY); }

extern Reprog *regcomplit9(char *s) { return regcomp1(s, 1, ANY); }

extern Reprog *regcompnl9(char *s) { return regcomp1(s, 0, ANYNL); }
 ...
 */
static int regexec1(const Reprog *progp, /* program to run */
...
static int regexec2(const Reprog *progp, /* program to run */
                    char *bol,           /* string to run machine on */
                    Resub *mp,           /* subexpression elements */
                    int ms,              /* number of elements at mp */
                    Reljunk *j) {
...
extern int regexec9(const Reprog *progp, /* program to run */
                    char *bol,           /* string to run machine on */
                    Resub *mp,           /* subexpression elements */
                    int ms)              /* number of elements at mp */
{
...
extern void regsub9(char *sp,            /* source string */
 ...
 *		<0 if we ran out of _relist space
 */
static int rregexec1(const Reprog *progp, /* program to run */
...
static int rregexec2(const Reprog *progp, /* program to run */
                     Rune *bol,           /* string to run machine on */
                     Resub *mp,           /* subexpression elements */
                     int ms,              /* number of elements at mp */
                     Reljunk *j) {
...
extern int rregexec9(const Reprog *progp, /* program to run */
                     Rune *bol,           /* string to run machine on */
                     Resub *mp,           /* subexpression elements */
                     int ms)              /* number of elements at mp */
{
...
/* substitute into one string using the matches from the last regexec() */
extern void rregsub9(Rune *sp,            /* source string */
  ...
  while (*sp != '\0') {
    if (*sp == '\\') {
      ...
      if (mp[0].s.rsp != 0 && mp != 0 && ms > 0)
        if (mp[0].s.rsp != 0)
          for (ssp = mp[0].s.rsp; ssp < mp[0].e.rep; ssp++)
            if (dp < ep)
              *dp++ = *ssp;
    } else {
      if (dp < ep)
