...
static size_t Table_Step(struct Table *t) {
  return sizeof(uint64_t) + sizeof(struct Header) + t->ksize +
...
static var Table_Key(struct Table *t, uint64_t i) {
  return (char *)t->data + i * Table_Step(t) + sizeof(uint64_t) +
...
static void Table_Set_Move(var self, var key, var val, bool move) {
  ...
  if (move) {
    ...
    memcpy((char *)t->sspace0 + sizeof(uint64_t) + sizeof(struct Header) +
               t->ksize,
    ...
    struct Header *khead =
        (struct Header *)((char *)t->sspace0 + sizeof(uint64_t));
    struct Header *vhead =
        (struct Header *)((char *)t->sspace0 + sizeof(uint64_t) +
  ...
  while (true) {
    ...
    if (j >= p) {
      memcpy((char *)t->sspace1, (char *)t->data + i * Table_Step(t),
             Table_Step(t));
      memcpy((char *)t->data + i * Table_Step(t), (char *)t->sspace0,
             Table_Step(t));
