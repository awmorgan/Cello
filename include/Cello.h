...
#define method(X, C, M, ...)                                                   \
  ((struct C *)method_at_offset(X, C, offsetof(struct C, M), #M))              \
      ->M(X, ##__VA_ARGS__)
...
#define type_method(T, C, M, ...)                                              \
  ((struct C *)type_method_at_offset(T, C, offsetof(struct C, M), #M))         \
      ->M(__VA_ARGS__)
...
#define $(T, ...)                                                              \
  ((struct T *)memcpy(alloc_stack(T), &((struct T){__VA_ARGS__}),              \
                      sizeof(struct T)))
...
#define slice_in(...)                                                          \
  slice_stack($(Slice, NULL, $(Range, $I(0), 0, 0, 0)), tuple(__VA_ARGS__))
